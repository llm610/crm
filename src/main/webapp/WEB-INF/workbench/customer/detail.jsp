...
<head>
	...
	$(function(){
		...
		function loadCustomer() {
			$.ajax({
				url: "/customer/get.json?id=${param.id}",
				success: function (data) {
					...
					$("#customerLink").prop("href", data.website);
				}
			})
		}
		loadCustomer();
		...
		function loadRemarks() {
			...
			$.ajax({
				url: "/customer/getRemarks.json?id=${param.id}",
				success: function (data) {
					$(data).each(function () {
						var item = '\
						<div class="remarkDiv" style="height: 60px;">\
							...
							<div style="position: relative; top: -40px; left: 40px;" >\
								...
								<b>{{name}}</b> <small style="color: gray;"> {{noteTime}} 由{{notePerson}}</small>\
						...
						for(var key in this) {
							item = item.replace("{{" + key + "}}", this[key]||"");
						}
						item = item.replace("{{name}}", customer.name);

						$("#remarkDiv").before(item);
					})
				}
			})
		}
		...
		function loadContacts() {
			$("#contactsBody").html("");
			$.ajax({
				url: "/customer/getContacts.json?id=${param.id}",
				success: function (data) {
					$(data).each(function () {
						var item = '\
						<tr>\
							...
							<td>{{mphone}}</td>\
							<td>\
								<a delete="{{id}}" data-toggle="modal" data-target="#removeContactsModal" style="text-decoration: none;cursor:pointer;">\
		...
		<%--<c:forEach items="${stage2possiList}" var="entry">
		stage2possiObj.${entry.key} = ${entry.value};
		</c:forEach>--%>
		...
		function loadTransactions() {
			$("#tranBody").html("");
			$.ajax({
				url: "/customer/getTransactions.json?id=${param.id}",
				success: function (data) {
					$(data).each(function () {
						var item ='\
						<tr>\
							...
							<td>{{stage}}</td>\
							<td>{{pissibility}}</td>\
							<td>{{expectedClosingDate}}</td>\
							<td>{{type}}</td>\
							<td>\
								<a delete="{{id}}" data-toggle="modal" data-target="#removeTransactionModal" style="text-decoration: none;cursor:pointer;">\
								<span class="glyphicon glyphicon-remove"></span>删除</a>\
							</td>\
						</tr>';
						for(var key in this) {
						...
						item = item.replace("{{pissibility}}", stage2possiObj[this.stage]);
						$("#tranBody").append(item);
					});
				}
			})
		}
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3><span text="name"></span> <small><a id="customerLink" href="" text="website" target="_blank"></a></small></h3>
		</div>
	...
	<div style="position: relative; top: -70px;">
		<div style="position: relative; left: 40px; height: 30px;">
			...
			<div style="width: 300px;position: relative; left: 200px; top: -20px;"><b><span text="owner"></span></b></div>
			<div style="width: 300px;position: relative; left: 450px; top: -40px; color: gray;">名称</div>
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b><span text="name"></span></b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 10px;">
			<div style="width: 300px; color: gray;">公司网站</div>
			<div style="width: 300px;position: relative; left: 200px; top: -20px;"><b><span text="website"></span></b></div>
			<div style="width: 300px;position: relative; left: 450px; top: -40px; color: gray;">公司座机</div>
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b><span text="phone"></span></b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 20px;">
			<div style="width: 300px; color: gray;">创建者</div>
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b><span text="createBy"></span>&nbsp;&nbsp;</b><small style="font-size: 10px; color: gray;"><span text="createTime"></span></small></div>
			<div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 30px;">
			<div style="width: 300px; color: gray;">修改者</div>
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b><span text="editBy"></span>&nbsp;&nbsp;</b><small style="font-size: 10px; color: gray;"><span text="editTime"></span></small></div>
			<div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
        ...
        <div style="position: relative; left: 40px; height: 30px; top: 50px;">
            ...
            <div style="width: 300px;position: relative; left: 200px; top: -20px;"><b><span text="nextContactTime"></span></b></div>
	...
	<!-- 备注 -->
	<div id="remarkList" style="position: relative; top: 10px; left: 40px;">
		<div class="page-header">
			<h4>备注</h4>
		</div>
